...
**goto** is a command-line shortcuts manager that lets users quickly go to predefined local files and directories as well as website links. The program is entirely contained within a single script, `goto.sh`. **goto** keeps track of shortcuts internally as keywords linked in a tree structure (represented in a permanent JSON file). This way, you can define a shortcut that is nested inside another, allowing for the grouping of shortcuts by topic. You can also maintain multiple shortcuts that have the same keyword as long as they belong to different branches of the shortcuts tree.
